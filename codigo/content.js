...
// ✅ MEJORADO: Funciona independientemente en múltiples pestañas

console.log("🚀 [AutoTag] Script cargado. Esperando comandos...");

// ✅ NUEVA: Variables de estado independiente por pestaña
let contentScriptReady = false;

// ✅ NUEVA: Registrar que este content script está listo
window.addEventListener('load', () => {
  console.log("[AutoTag] ✅ Página completamente cargada - Content script listo para comandos");
  ...
  chrome.runtime.sendMessage({
    ...
    console.log("[AutoTag] Background.js no está disponible aún");
  });
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "abrirChats") {
    ...
    if (typeof chatOpener !== 'undefined') chatOpener.startChatIteration();
  }
  ...
  if (message.action === "tagearChatsV2") {
    ...
    if (typeof chatTagger !== 'undefined') chatTagger.startTagIteration();
  }
  ...
  if (message.action === "observarChats") {
    ...
    if (typeof chatObserver !== 'undefined') chatObserver.startObserveIteration();
  }
});
